...
from classes import Provider, Member, Service, ServiceRecord, MemberStatus
...
def test_insert_and_get_provider(db, sample_provider):
    ...
    assert retrieved_provider._name == sample_provider._name
...
def test_service_record_with_invalid_references(db):
    ...
    invalid_record = ServiceRecord(
        dateReceived=datetime.now(),
        serviceDate=datetime.now().date(),
        provider=Provider("999999999", "Invalid", "123 St", "City", "ST", "12345"),
        member=Member("888888888", "Invalid", "User", "123 St", "City", "ST", "12345", MemberStatus.VALID),
        service=Service("777777", "Invalid Service", 50.00),
        comments="Test comment",
        fee=50.00
    )
...
def test_long_values_truncation():
    ...
    long_name_provider = Provider(
        providerNumber="123456789",
        name="ThisIsAReallyReallyReallyLongNameThatExceedsTheLimit",
        streetAddress="123 Main St",
        city="Portland",
        state="OR",
        zipCode="97201"
